...
const listApiFetch = function (...args){
    ...
    return fetch(...args)
        .then(res=>{
            if(!res.ok){
                //if response if not 200x start building error object
                error = {code: res.status};

                //if response is not of type JSON, put status text in err object and
        ...
        //otherwise, return json as normal resolved Promise
        return data;
        });
};
